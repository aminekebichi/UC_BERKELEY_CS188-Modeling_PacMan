...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # all edges given in the diagram
    ...
    # observation domains 
    max_distance = (X_RANGE - 1) + (Y_RANGE - 1) + MAX_NOISE # max true distance + max noise
    observation_distances = list(range(max_distance + 1))
    variableDomainsDict[OBS0] = observation_distances
    variableDomainsDict[OBS1] = observation_distances
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        if len(currentFactorsList) > 0:
            ...
            finalFactor = Factor([], [], bayesNet.variableDomainsDict())
            finalFactor.setProbability({}, 1.0)
...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # dividing each value by total value
        for key in self:
    ...
    def sample(self):
        ...
        # random number between 0 and totalValue
        randomPick = random.random() * totalValue
        
        # finds corresponding key based on probabilities
        ...
        # returns last key if no key is found
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition:
            if noisyDistance is None:
                return 1.0 # (ghost is in jail)
            else:
                return 0.0 # (ghost is not in jail)
        ...
        # returns probability of noisy distance given true distance
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # Pacman's position and jail position
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        
        # using Bayes rule, updates beliefs for all possible positions using Bayes rule
        for position in self.allPositions:
            ...
            self.beliefs[position] = observationProb * priorBelief
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # belief distribution for time t+1
        newBeliefs = DiscreteDistribution()
        
        # each position where the ghost could currently be (at time t)
        for oldPos in self.allPositions:
            oldBelief = self.beliefs[oldPos]
            
            # distribution over new positions
            newPosDist = self.getPositionDistribution(gameState, oldPos)
                
            # new position at time t+1 
            for newPos, transitionProb in newPosDist.items():
                newBeliefs[newPos] += transitionProb * oldBelief
